...
void grad12( float *I, float *Gx, float *Gy, int h, int w, int x ) {
...
void grad22( float *I, float *Gx, float *Gy, int h, int w, int d ) {
  int o, x, c, a=w*h; for(c=0; c<d; c++) for(x=0; x<w; x++) {
    o=c*a+x*h; grad12( I+o, Gx+o, Gy+o, h, w, x );
  }
}
...
void gradMag2( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  ...
  float *acost = acosTable2(), acMult=10000.0f;
  ...
  for( x=0; x<w; x++ ) {
    ...
    for(c=0; c<d; c++) {
      grad12( I+x*h+c*w*h, Gx+c*h4, Gy+c*h4, h, w, x );
      for( y=0; y<h4/4; y++ ) {
        y1=h4/4*c+y;
...
void gradMagNorm2( float *M, float *S, int h, int w, float norm ) {
...
void gradQuantize2( float *O, float *M, int *O0, int *O1, float *M0, float *M1,
  ...
  int bin, int nOrients, int softBin, bool full )
{
  ...
  for( x=0; x<w0; x++ ) {
    ...
    gradQuantize2(O+x*h,M+x*h,O0,O1,M0,M1,nb,h0,sInv2,nOrients,full,softBin>=0);
...
float* hogNormMatrix2( float *H, int nOrients, int hb, int wb, int bin ) {
...
void hogChannels2( float *H, const float *R, const float *N,
  ...
  int nOrients, int softBin, bool full, float clip )
{
  ...
  gradHist2( M, O, R, h, w, binSize, nOrients, softBin, full );
  // compute block normalization values
  N = hogNormMatrix2( R, nOrients, hb, wb, binSize );
  // perform four normalizations per spatial block
  hogChannels2( H, R, N, hb, wb, nOrients, clip, 0 );
  wrFree(N); wrFree(R);
}
  ...
  int nOrients, int softBin, float clip )
{
  ...
  gradHist2( M, O, R1, h, w, binSize, nOrients*2, softBin, true );
  ...
  // compute block normalization values
  N = hogNormMatrix2( R2, nOrients, hb, wb, binSize );
  // normalized histograms and texture channels
  hogChannels2( H+nbo*0, R1, N, hb, wb, nOrients*2, clip, 1 );
  hogChannels2( H+nbo*2, R2, N, hb, wb, nOrients*1, clip, 1 );
  hogChannels2( H+nbo*3, R1, N, hb, wb, nOrients*2, clip, 2 );
